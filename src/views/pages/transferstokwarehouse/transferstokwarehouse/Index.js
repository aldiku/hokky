...
const TransferStokWarehouse = () => {
  const token = localStorage.token;
  const warehouse = parseInt(localStorage.warehouse);
  const redirectPrefix = `/admin/transfer-warehouse-stock/edit/`;
  ...
  const allInfo = JSON.parse(localStorage.allinfo);
	const createButton = String(allInfo.privileges.filter((i) => i.privilege_name === "Tab Transfer Eksternal").map((p) => p.create_access));
          ...
          <Card className="bg-secondary shadow">
              <CardHeader className="bg-white border-0">
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <h3>Transfer Eksternal</h3>
                  {createButton && createButton === "1" && (
                  <div style={{ textAlign: 'right' }}>
                    <Link className="btn btn-info" to="/admin/transfer-warehouse-stock/create">
                        ...
                        <Row md="12">
                          <Col md="3">
                            <FormGroup>
                              ...
                              <Input
                                type="text"
                                placeholder="Masukan Kode Transfer Stok"
                                value={tw_code}
...
export default TransferStokWarehouse;