...
export default function TransferStokWarehousePage() {
	...
	const transferStockWarehouse = String(allInfo.privileges.filter((i) => i.privilege_name === "Tab Transfer Eksternal").map((p) => p.read_access));
	const validasiManajer = String(allInfo.privileges.filter((i) => i.privilege_name === "Tab Validasi TE Kepala Toko").map((p) => p.read_access));
	const validasiDirektur = String(allInfo.privileges.filter((i) => i.privilege_name === "Tab Validasi TE Direktur").map((p) => p.read_access));
	const cetak = String(allInfo.privileges.filter((i) => i.privilege_name === "Tab Cetak TE").map((p) => p.read_access));
