...
const SuratJalanTransfer = () => {
  const token = localStorage.token;
  const warehouse = parseInt(localStorage.warehouse);
  const redirectPrefix = `/admin/transfer-surat-jalan/edit/`;
  const redirectPrefix1 = `/cetak/transfer-surat-jalan/cetak-cetak/`;
  ...
  const allInfo = JSON.parse(localStorage.allinfo);
	const createButton = String(allInfo.privileges.filter((i) => i.privilege_name === "Tab Surat Jalan TE").map((p) => p.read_access));
  ...
  const updateDataTable = (page, perPage, sort, uomCode, description) => {
    getSuratJalanTransfer(page, perPage, sort, uomCode, description);
    setPage(page);
    setPerpage(perPage);
  ...
  useEffect(() => {
    getSuratJalanTransfer(page, perPage, currentSort);
  }, []);

  // fungsi dari ambil data
  const getSuratJalanTransfer = (page, perPage, currentSort) => {
          ...
          <Card className="bg-secondary shadow">
              <CardHeader className="bg-white border-0">
                <div style={{ display: "flex", justifyContent: "space-between" }}>
                  <h3>Surat Jalan Transfer</h3>
                  <div style={{ textAlign: 'right' }}>
                    <Link className="btn btn-info" to="/admin/transfer-surat-jalan/create">
...
export default SuratJalanTransfer;