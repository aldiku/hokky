...
export default function KasirIndex() {
    ...
    const handleConfEditable = () => {
		axios.post(`${process.env.REACT_APP_API_BASE_URL}/password-operasional`,{page:1,per_page:10,"warehouse_id":parseInt(warehouse)},{headers})
				.then((res) => {
													...
													<i className="fa fa-search mr-2"></i>
														...
														{isSearchShoww && queryy && (
															<Card className="position-absolute boxShadow" style={{ top:"50px",maxHeight: "15.5rem", zIndex:'1',overflowY: "auto", paddingTop: "1rem", position: "relative" }}>
																<div style={{ position: "absolute", top: "2.5px", right: "1rem", cursor: "pointer", fontSize: "1rem", zIndex:1 }}>
																	<i className="fas fa-window-close text-danger" onClick={() => setIsSearchShoww(false)}></i>
																</div>
																{allItemm?.response ? (
																	allItemm.response.map((item) => (
																		<Card key={item.id} className="bgSearch p-2 mb-0" onClick={() => {
                                                                            searchh(item);
                                                                        }} style={{height:'60px'}}>
                                                                    ...
                                                                    <div className="col text-center" style={{width: '300px'}}>
										...
										<Table className="table-sm">
											<thead>
												<tr>
													...
													<th className="px-1"># {editable?<Button color="danger" onClick={() => setEditable(false)} size="sm"><i className="fa fa-lock"></i> Close Edit</Button>
												:<Button color="danger" onClick={() => setIsConfEditableOpen(true)} size="sm"><i className="fa fa-lock"></i> Edit Harga</Button>}
                                                </th>
												</tr>
											</thead>
												<tbody>
												{
													savedItems.map((savedItem, key) => {
														return (
															<tr key={key}>
																<td className="px-2 align-middle">{savedItem.item_name}
                                                                </td>
																...
																<td className="px-2 align-middle" >
																		<>{formatRupiah(savedItem.harga * savedItem.qty - savedItem.diskon_nominal -(savedItem.diskon_persen*savedItem.harga * savedItem.qty/100))}</>
																</td>
																<td className="px-2 align-middle">
																	{editingItem[key].editing ? (
																		<>
																			<Button className="btn-sm" color="warning" onClick={() => changeItemDataTable({
																				index: key,
																		...
																		<>
																			<Button className="btn-sm" color="warning" onClick={() => changePriceStatus(key, true)}><i className="fa fa-edit"></i></Button>
																			<Button className="btn-sm" color="danger" onClick={() => deleteItem(savedItem.item_id)}><i className="fa fa-trash"></i></Button>
																		</>
																	)}
																</td>
															</tr>
									...
									<Col md={3}>
										<div className="card text-center p-2 mb-2">
											...
											<div className="d-flex">
												...
												<a href="/admin/customer/create" target='_blank' className="btn btn-warning btn-sm align-middle">
										...
										{ (pengiriman >= 2 ) ? 
											(
                                                ...
                                                <TextField id="alamat" label="Alamat Pengantaran" className="w-100 my-2" variant="outlined" 
                                                    type='text'
                                                    size="small" 
                                                    value={alamatlain}
                                                    onChange={(e) => {
										...
										<div className="d-flex justify-content-between my-1">
											<div className="btn btn-danger" onClick={() => { if (window.confirm('Tutup Kasir sekarang ?')) history.push("/admin/kasir-sales-order/detail") } } >
												Closing Cashsier
											</div>
											<Button
												className="col"
