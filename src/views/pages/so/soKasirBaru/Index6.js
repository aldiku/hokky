...
export default function CreateSalesOrder() {
					...
					<Card className="position-sticky boxShadow" style={{ maxHeight: "15.5rem", overflowY: "auto", paddingTop: "1rem", position: "relative" }}>
						...
						{allItemm?.response ? (
							allItemm.response.map((item) => (
								<CardBody key={item.id} style={{ minHeight: "5rem", padding: "1rem" }} className="bgSearch" onClick={() => {saveItemOrUpdate(item);setItemIdd(item.id);setQueryy('');setAddQty(1);setIsSearchShoww(false);}}>
									<div>
	...
	const saveItemOrUpdate = (item) => {
		...
		if(oldobj===undefined){
			...
			return setTotalPrice(totalPrice + item.price * addQty - diskonnominal);
		}else{
			let findedindex = savedItems.findIndex(e=>e.item_id===item.id);
			let updateList = savedItems;
			let qtyupdate = parseInt(updateList[findedindex].qty)+parseInt(addQty);
			updateList[findedindex] = {...updateList[findedindex], qty: qtyupdate};
			setSavedItems(updateList);
			return setTotalPrice(totalPrice + updateList[findedindex].harga * qtyupdate - diskonnominal);
		};
		// const headers = {
		// 		"Content-Type": "application/json",
		// 		Authorization: `Bearer ${token}`,
