...
export default function CreateSalesOrder() {
    ...
    useEffect(() => {
		setTotalPrice1( totalPrice - ppnnew  + ongkir1 );
	}, [totalPrice, ppnnew, ongkir1]);
	...
	const getById2 = () => {
	    ...
	    axios
	      ...
	      .then((data) => {
			...
			setNamaCustomer(data.data.response.name)
	      })
	      .catch(function (error) {
	        console.log(error);
	      });
	...
	const getBankPayment1 = () => {
		const headers = {
			"Content-Type": "application/json",
			Authorization: `Bearer ${token}`,
		};
		axios
			.get(`${process.env.REACT_APP_API_BASE_URL}/bank/get/${payment_method1}`, { headers })
			.then((data) => {
				setNamaPaymentMethod(data.data.response.bank_name);
			})
			.catch(function (error) {
	...
	return (
		<>
								...
								<Row className="mb-3">
										...
										<Col xs={3}>
                                            ...
                                            autoComplete="off" 
												...
												<option value={1} onClick={() => setJenisTransaksi(1)}>Ambil Sendiri</option>
												<option value={2} onClick={() => setJenisTransaksi(2)}>
													Delivery
												</option>
												<option value={3} onClick={() => setJenisTransaksi(3)}>Kurir</option>
											</Input>
										</Col>
										<Col xs={3}>
										<InputGroup>
			...
			customer={customer} alamatcustomer={alamatcustomer} alamatlain={alamatlain} namacustomer={namacustomer}
