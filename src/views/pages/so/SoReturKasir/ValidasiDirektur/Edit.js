...
export default function ValidasiReturSoKepalaTokoBaru(props) {
    const token = localStorage.token;
    const username = localStorage.username;
    const warehouse = localStorage.warehouse;
    let history = useHistory();
    const [statusaf, setStatusAf] = useState("");
    const [statusaf1, setStatusAf1] = useState("");
    const [statusaf2, setStatusAf2] = useState("");
  ...
  const getById = () => {
    ...
    axios
      ...
      .then((data) => {
        ...
        // setCodeInvoice(data.item.code_transaction);
        ...
        setStatusAf(data.data.response.status_ar1);
        setStatusAf1(data.data.response.status_ar2);
...
const getCustomer = (id) => {
   ...
   function EditData() {
    ...
    let data = {
        ...
        status_ar1: parseInt(statusaf),
        status_ar2: parseInt(statusaf1),
        admin_retur2: statusaf,
        status_d:parseInt(statusaf2),
    ...
    axios
        .post(
          `${process.env.REACT_APP_API_BASE_URL}/so-retur/cashier/update/${props.match.params.id}`,
          data,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        .then(function (response) {
          history.push("/admin/kasir-retur");
        })
        .then((json) => {
          setLoading(false);
        })
            ...
            <CardBody>
              ...
              <Card className="bg-secondary shadow">
                ...
                <Button color="danger" onClick={toggle}>
                      ...
                      <Link className="btn btn-info" to="/admin/so-retur">
                        Kembali
                      </Link>
                </CardFooter>
