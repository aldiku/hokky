...
const PdfReportJurnalUmum = ({data}) => {
    ...
    useEffect(() => {
        getPenawaranSo(data.start ,data.end, data.statusph);
        return () => {
            setAllPenawaranSo([]); 
        };
    }, []);

    async function getPenawaranSo( start=start, end=end, statusph=statusph) {
        let filter = { 
          per_page: 1000,
    ...
    <Document>
        ...
        <Table data={allPenawaranSo} style={s.my1}>
                <TableHeader textAlign={"center"}>
                    <TableCell style={s.fs08}>Tanggal Transaksi</TableCell>
                    <TableCell style={s.fs08}>Kode Transaksi</TableCell>
                    {/* <TableCell weighting={0.3} style={s.fs08}>QTY</TableCell>
                    <TableCell weighting={0.4} style={s.fs08}>Total</TableCell> */}
                    <TableCell style={s.fs08}>Akun</TableCell>
                    {/* <TableCell style={s.fs08}></TableCell> */}
                    <TableCell style={s.fs08}>Debet</TableCell>
                    <TableCell style={s.fs08}>Kredit</TableCell>
                    <TableCell style={s.fs08}>Saldo</TableCell>
                </TableHeader>
                <TableBody  textAlign={"center"} >
                    <DataTableCell getContent={(r) => r.tgl_transaksi}  style={s.fs08}/>
                    <DataTableCell getContent={(r) => r.transaction_code}  style={s.fs08}/>
                    ...
                    <DataTableCell getContent={(r) => r.account_name} style={[s.fs08,s.textCenter]}/>
                    {/* <DataTableCell getContent={(r) => r.potongan} style={[s.fs08,s.textCenter]}/> */}
                    <DataTableCell getContent={(r) => r.debet_total} style={[s.fs08,s.textCenter]}/>
                    <DataTableCell getContent={(r) => r.kredit_total} style={[s.fs08]}/>
                    <DataTableCell getContent={(r) => r.saldo_total} style={[s.fs08]}/>
                </TableBody>
            </Table>
...
export default PdfReportJurnalUmum;