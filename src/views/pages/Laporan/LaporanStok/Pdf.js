...
const PdfReportStok = ({data}) => {
    ...
    useEffect(() => {
        getPenawaranSo(data.start ,data.end, data.status);
        return () => {
            setAllPenawaranSo([]); 
        };
    }, []);

    async function getPenawaranSo( start=start, end=end, status=status) {
        let filter = { 
          per_page: 1000,
