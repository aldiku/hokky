...
const Coa = () => {
  ...
  const getCoa = async (page, perPage, currentSort, account_name = null) => {
    let filter = { page: page, per_page: perPage };
    if (account_name !== null) {
      filter = Object.assign(filter, { account_name: account_name });
    }
    const data = filter;
    const headers = {
    ...
    const res = await axios.post(
      `${process.env.REACT_APP_API_BASE_URL}/account/page`,
      data,
      { headers }
    );
