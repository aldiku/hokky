...
export default function Userss(props) {
  ...
  const handleEditQty = (index, value, type) => {
    console.log(`index : ${index}, val : ${value}, type: ${type}`);

    var newval = value == "1" ? 0 : 1;
    if (type == "create") {
      savedItems[index] = {
        ...savedItems[index],
        create_access: newval,
      };
    }
    if (type == "read") {
      savedItems[index] = {
    ...
    if (type == "update") {
      savedItems[index] = {
        ...savedItems[index],
        update_access: newval,
      };
    }
    if (type == "delete") {
      savedItems[index] = {
        ...savedItems[index],
        delete_access: newval,
      };
    }
  ...
  const savePreveliege = (item, key) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_API_BASE_URL}/users/makro-privileges/update/${item.user_id}`,
        data,
        { headers }
      )
      .then((response) => {
                  ...
                  <Row md="12">
                    ...
                    <Col md="3">
                      <div style={{ textAlign: "right" }}>
                        <Button color="info" onClick={toggle} disabled>
