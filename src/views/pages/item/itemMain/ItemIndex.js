...
export default function ItemIndex() {
	...
	const getById = (id) => {
	    ...
	    axios.get(
	        `${process.env.REACT_APP_API_BASE_URL}/items/${id}`,{headers }
	      ).then((data) => {
			setLoading(false);
	...
	const getSubMerek = (id) => {
		...
		axios
			.get(`${process.env.REACT_APP_API_BASE_URL}/sub-merek/list?merek_id=${merekId}`, { headers })
			.then((data) => {
							...
							<Form onSubmit={handleSubmit}>
								...
								<Card className="position-sticky" style={{ top: 0, zIndex: "5" }}>
									<CardBody className="ml-3 pb-0">
										<Row md="12">
											<Col md="5">
												<Button onClick={() => setPage(1)} color="danger" type="button">
													First
												</Button>
												<Button onClick={() => setPage((page) => page - 1)} disabled={page === 1} color="success" type="button">
													<i className="ni ni-bold-left" /> Prev
												</Button>
												<Button onClick={() => setPage((page) => page + 1)} disabled={page >= totalPages-1} color="success" type="button">
											...
											allItem.response.map((item) => (
												<CardBody key={item.id} className="bgSearch border-bottom p-2 mb-2" onClick={() => searchShow(item)}style={{minHeight: 'auto'}}>
													<div className=" d-flex align-items-center">
														...
														<div className="ml-3">
															<div>
																<b>Price:</b> {new Intl.NumberFormat().format(item.price)}
															</div>
															<div>
