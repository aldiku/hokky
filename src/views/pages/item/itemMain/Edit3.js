...
export default function EditItem(props) {
	...
	useEffect(() => {
		setHMD(hargajualminimal + marginglobalnominal - diskonnominal );
	}, [hargajualminimal,marginglobalnominal,diskonnominal]);

	// Harga - PPN
	useEffect(() => {
		// ppnJual === "pajakNominal" && setPajakJual((pajakjualnominal / hmd) * 100);
		// pajakjual && ppnJual === "pajakPersen" && 
		setPajakJualNominal(hmd * (pajakjual / 100));
	}, [hargajualminimal, pajakjual, pajakjualnominal]);

	// Harga - Harga Jual
	useEffect(() => {
			setHargaJual(hargajualminimal + marginglobalnominal - diskonnominal + pajakjualnominal);
