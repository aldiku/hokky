...
export default function EditItem() {
	  ...
	  const getById = () => {
	    ...
	    axios
	      ...
	      .then((data) => {
	        ...
	        // setMerek(data.data.response.items.merek_id);
	        // setSubMerek(data.data.response.items.submerek_id);
			getSubCategory(
			  ...
			  getSubMerek(
				...
				data.data.response.items.submerek_id
				);
	...
	const search = async () => {
		if (query) {
			const res = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/items-po`, { item_info: query , warehouse_id: parseInt(warehouseId) }, { headers });
									...
									<Card className="boxShadow" style={{ maxHeight: "15.5rem", overflowY: "auto", paddingTop: "1rem", position: "relative" }}>
										...
										{allItem?.response ? (
											allItem.response.map((item) => (
												<CardBody key={item.id} style={{ minHeight: "8rem", padding: "1rem" }} className="bgSearch" onClick={() => searchShow(item)}>
													<div>
													...
													<div>
														<b>Stok:</b> {item.stock ? item.stock : "(Not available)"}
													</div>
														...
														<CardGroup>
                                                            ...
                                                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
																<Col md="10" align="center">
																	<Card>
																		<CardImg className="photo" alt="Belum Ada Gambar" src={image1} center/>
																	</Card>
																</Col>
															</Row>
														</CardGroup>
