...
const Jurnal = () => {
  ...
  const getJurnal = async (page, perPage, currentSort, account_id = 0, coa_id = 0, type = 0 ) => {
    let filter = {
      page: page,
      per_page: perPage,
    ...
    const res = await axios.post(
      `${process.env.REACT_APP_API_BASE_URL}/account-journal/page`,
      data,
      { headers }
    );
    setAllJurnal(res.data.response);
    // setNama(res.data.response.account_code)
  ...
  return (
    <div>
      {alert}
      <SimpleHeader name="Jurnal" parentName="Master" />
