	...
	const search = async () => {
		if (Number(query) > 0) {
			...
			if (res.data.status !== 404) setAllItem(res.data);
				...
				const res = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/warehouse/all`, { warehouse_code: query }, { headers });
				if (res.data.status !== 404) setAllItem(res.data);
                ...
                {isSearchShow && query && (
                    <Card className="boxShadow" style={{ maxHeight: "15.5rem", overflowY: "auto", paddingTop: "1rem", position: "relative" }}>
                        ...
                        {allItem?.response ? (
                            ...
                            <div className="text-center mb-3 text-danger">Cabang "{query}" tidak ada bosku!</div>
                        )}
                    </Card>
                )}
                <CardBody>
                                            ...
                                            <CardGroup>
                                                ...
                                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
																<Col md="10" align="center">
																	<Card>
																		<CardImg className="photo" alt="Belum Ada Gambar" src={image} center/>
																	</Card>
																</Col>
                                                </Row>
                                            </CardGroup>
