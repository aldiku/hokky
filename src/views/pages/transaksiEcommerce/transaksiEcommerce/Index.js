...
const TransaksiEcommerce = () => {
  const token = localStorage.token;
  const warehouse = parseInt(localStorage.warehouse);
  const redirectPrefix = `/admin/transaction/validate/`;
  const [rowIndex, setRowIndex] = useState(0);
  const [TransactionHistory, setAllTransactionHistory] = useState([]);
  ...
  const updateDataTable = (page, perPage, sort, uomCode, description) => {
    getPenawaranSo(page, perPage, sort, uomCode, description);
    setPage(page);
    setPerpage(perPage);
  ...
  const getPenawaranSo = (page, perPage, currentSort) => {
    ...
    axios
      .post(`${process.env.REACT_APP_API_BASE_URL}/transaction/erp/ongoing`, data, {
        headers,
      })
      .then((data) => {
        setAllTransactionHistory(data.data.response);
...
export default TransaksiEcommerce;